...
class SettingsPage(QtWidgets.QWidget):
    ...
    def _build(self) -> None:
        ...
        tabs.addTab(tab_personal, t_local['personal_tab'])
        ...
        self.lbl_status = QtWidgets.QLabel(t_local['status'])
        ...
        for code in self._status_codes:
            ...
            visible = t_local.get(code, code) if isinstance(t_local, dict) else code
        ...
        self.edit_avatar.setPlaceholderText(t_local['avatar_placeholder'])
        self.btn_pick_avatar = NavigationButton(t_local['pick_avatar'])
        self.lbl_avatar = QtWidgets.QLabel(t_local['avatar'])
        pform.addRow(self.lbl_avatar, self._row_widget(self.edit_avatar, self.btn_pick_avatar))
        try:
            self.key_section = KeyPage()
            group = QtWidgets.QGroupBox(t_local['key_section'])
        ...
        tabs.addTab(tab_general, t_local['general_tab'])
        ...
        self.chk_debug = QtWidgets.QCheckBox(t_local['debug'])
        self.chk_trace = QtWidgets.QCheckBox(t_local['trace'])
        self.lbl_lang = QtWidgets.QLabel(t_local['lang'])
        self.lbl_encoding = QtWidgets.QLabel(t_local['encoding'])
        self.lbl_theme = QtWidgets.QLabel(t_local['theme'])
        gform.addRow(self.lbl_lang, self.cmb_lang)
        self.btn_enc_test = NavigationButton(t_local['encoding_selftest'])
        ...
        tabs.addTab(tab_net, t_local['network_tab'])
        ...
        self.edit_bind.setPlaceholderText(t_local['bind_ip'])
        self.edit_mask = QtWidgets.QLineEdit()
        self.edit_mask.setPlaceholderText(t_local['subnet_mask'])
        self.edit_keepalive = QtWidgets.QLineEdit()
        self.edit_keepalive.setPlaceholderText(t_local['keepalive'])
        self.edit_expire = QtWidgets.QLineEdit()
        self.edit_expire.setPlaceholderText(t_local['expire'])
        self.lbl_iface = QtWidgets.QLabel(t_local['iface'])
        self.lbl_keepalive = QtWidgets.QLabel(t_local['keepalive'])
        self.lbl_expire = QtWidgets.QLabel(t_local['expire'])
        self.lbl_bind = QtWidgets.QLabel(t_local['bind_ip'])
        self.lbl_mask = QtWidgets.QLabel(t_local['subnet_mask'])
        ...
        tabs.addTab(tab_files, t_local['files_tab'])
        fform = QtWidgets.QFormLayout(tab_files)
        fform.setContentsMargins(12, 12, 12, 12)
        fform.setSpacing(10)
        self.edit_dir = QtWidgets.QLineEdit()
        self.edit_dir.setPlaceholderText(t_local['download_dir'])
        self.btn_browse_dir = NavigationButton(t_local['browse'])
        self.edit_ss_dir = QtWidgets.QLineEdit()
        self.edit_ss_dir.setPlaceholderText(t_local['screenshot_dir'])
        self.btn_browse_ss = NavigationButton(t_local['browse_ss'])
        self.lbl_download = QtWidgets.QLabel(t_local['download_dir'])
        self.lbl_ss_dir = QtWidgets.QLabel(t_local['screenshot_dir'])
        ...
        self.btn_apply = NavigationButton(t_local['apply'])
        self.btn_logout = NavigationButton(t_local['logout_long'])
    ...
    def _pick_download_dir(self) -> None:
        t_local = self._translations
        directory = QtWidgets.QFileDialog.getExistingDirectory(self, t_local['pick_download_dir'])
        if directory:
    ...
    def _pick_avatar(self) -> None:
        ...
        path, _ = QtWidgets.QFileDialog.getOpenFileName(
            self,
            t_local['pick_avatar_dialog'],
            filter=t_local['images_filter'],
        )
        if path:
    ...
    def _pick_screenshot_dir(self) -> None:
        t_local = self._translations
        directory = QtWidgets.QFileDialog.getExistingDirectory(self, t_local['pick_screenshot_dir'])
        if directory:
    ...
    def update_personal_info(self, username: str, ip: str) -> None:
        ...
        t_local = getattr(self, '_translations', {})
        self.lbl_p_username.setText(f"{t_local['username']}：{self._personal_username}")
        self.lbl_p_ip.setText(f"{t_local['ip']}：{self._personal_ip}")
