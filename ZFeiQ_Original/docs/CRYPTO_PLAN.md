...
加密算法: HKDF‑SHA256 + AES‑256‑GCM（会话加密）。
握手与会话: 采用 HKDF‑only 的 KX1/KX2 握手（明文交换 seedA/seedB），双方按 IP 字典序决定 IKM 拼接顺序，经 HKDF 派生 32B 会话密钥；sid = SHA256(IKM)[0:8]。
指纹绑定: 收到公钥时计算 SHA256 指纹，若与对端广播宣布的指纹不一致会告警（但未阻断）。
加密封装: 统一为 ENC 帧：`ENC;sid=<b64>;ctr=<u64>;tag=<b64>;b64=<cipher>`；nonce = SHA256(sid||nonce_base||ctr)[0:12]；AAD = sid。
解密路径: 收到 ENC 帧后按 sid 查找会话，基于 ctr 派生 nonce 与验证 AAD；成功则输出明文并可选打印 `[cipher]` 行与 `[E-D OK]` 标签。
已知不足
...
级别 B（已采用，HKDF-only 会话密钥）：在首次往来时完成“明文种子交换 + HKDF 会话密钥派生”，后续消息统一使用 `ENC` 会话加密；nonce 使用 `SHA256(sid||nonce_base||ctr)[0:12]` 保证唯一性；周期性重键或基于计数阈值重键。
...
KX1（发起）:`KX1;ver=1;fp=<fpA>;seedA=<b64(32B)>`
KX2（响应）:`KX2;ver=1;fp=<fpB>;seedB=<b64(32B)>`
派生: seedA 与 seedB 均为 32B 随机；IKM 由 IP 字典序决定拼接顺序，经 HKDF(info="zfeiq-aes256gcm", L=32) 得到会话密钥。
编码: 会话加密头部统一为 `ENC;sid=<b64(8B)>;ctr=<u64>;tag=<b64>;b64=<cipher>`；nonce = H(sid||nonce_base||ctr)[0:12]；AAD = sid。
