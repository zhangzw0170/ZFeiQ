...
class ZFeiQOcr:
    ...
    def __init__(self):
        ...
        self.use_npu = False
        self.use_external_main = False
        self.external_main_path = None
        self.det_sess = None
        self.rec_sess = None
        self.ready = False
        ...
        candidates = [
            ...
            # 资源目录（dev-refactor 结构）
            os.path.abspath(os.path.join(PROJECT_ROOT, "..", "resource", "PPOCRv4"))
        ]
        ...
        # 记录外部主脚本路径（NPU 优先调用 legacy main.py）
        ext_main = os.path.join(self.base_path, "main.py")
        if os.path.isfile(ext_main):
            self.external_main_path = ext_main
    ...
    def _init_runtime(self):
        ...
        if 'aarch64' in self.arch or 'arm' in self.arch:
            try:
                ...
                print(f"[OCR] NPU init failed ({e}), trying external main.py...")
                # 优先尝试 legacy NPU 主脚本
                if self.external_main_path:
                    self.use_external_main = True
                    self.ready = True
                    return
    ...
    def run(self, img_path):
        ...
        try:
            # 如果使用外部 NPU 主脚本，则将图片复制到其默认路径并解析输出
            if self.use_external_main and self.external_main_path:
                import shutil, subprocess
                # legacy main.py 读取固定 IMG_PATH=./test.jpg
                work_dir = os.path.dirname(self.external_main_path)
                target_img = os.path.join(work_dir, "test.jpg")
                try:
                    shutil.copyfile(img_path, target_img)
                except Exception as e:
                    return f"OCR Error: prepare image failed ({e})"
                try:
                    proc = subprocess.run([sys.executable, self.external_main_path], cwd=work_dir, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
                    out = proc.stdout or ""
                    # 解析形如："   Box i: 'text' (Conf: 0.xx)"
                    lines = [ln.strip() for ln in out.splitlines()]
                    texts = []
                    for ln in lines:
                        if ln.startswith("Box") or ln.startswith("->"):
                            # 跳过计数行，仅解析 Box 行
                            pass
                        if "Box" in ln and "'" in ln:
                            try:
                                # 提取单引号中的内容
                                first = ln.find("'")
                                second = ln.find("'", first+1)
                                if first != -1 and second != -1:
                                    txt = ln[first+1:second]
                                    if txt:
                                        texts.append(txt)
                            except: pass
                    return "\n".join(texts) if texts else "No text detected."
                except Exception as e:
                    return f"OCR Error: external main failed ({e})"
