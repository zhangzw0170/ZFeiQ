...
- 安全（密钥）页：将密钥/加密相关内容独立为顶级“安全”标签页，脱离“个人”。
- 指纹展示：移除“刷新指纹”按钮（重生成密钥自动更新指纹），并采用 16‑bit 分组的小号等宽字体以提升可读性。
- 布局调整：左侧导航直接放入分割器并与聊天区等高；设置页在切换前放宽内部尺寸以避免强制拉高窗口。

## 近期更新（2025-11-28）

- 小幅版本更新（见 `CHANGELOG.md`）：
   - 语言切换即时生效，补齐若干翻译键并加入重复键检测脚本以纳入 CI 流程。
   - 新增关于（About）标签页：集中显示平台/版本/Logo 与本地化的源码仓库跳转按钮（GUI 专属）。
   - Key 页面与聊天页布局优化（指纹说明固定高度、聊天头部用户名/状态与 IP 同行、Key 组高度限制）。
   - 会话启动时改进公钥协商：主动广播 `cap=enc` 与 `fp=<sha256>` 指纹，并在需要时发送 `GETPUBKEY/ANSPUBKEY`。
   - 修复了若干由注释或重构引入的缩进错误并通过导入检查。

如需更详细的变更日志，请参阅仓库根目录的 `CHANGELOG.md`。
